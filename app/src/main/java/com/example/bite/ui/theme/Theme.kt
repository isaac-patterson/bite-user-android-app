...
private val LightColorPalette = BiteColors(
    brand = Shadow5,
...
private val DarkColorPalette = BiteColors(
    brand = Shadow1,
...
object BiteTheme {
    val colors: BiteColors
        @Composable
        get() = LocalBiteColors.current
}

/**
 * Bite custom Color Palette
 */
@Stable
class BiteColors(
...
@Composable
fun ProvideBiteColors(
    colors: BiteColors,
    content: @Composable () -> Unit
) {
    val colorPalette = remember { colors }
    colorPalette.update(colors)
    CompositionLocalProvider(LocalBiteColors provides colorPalette, content = content)
}

private val LocalBiteColors = staticCompositionLocalOf<BiteColors> {
    error("No BiteColorPalette provided")
}

/**
 * A Material [Colors] implementation which sets all colors to [debugColor] to discourage usage of
 * [MaterialTheme.colors] in preference to [BiteTheme.colors].
 */
fun debugColors(
    darkTheme: Boolean,
